...
**页面回收(page reclaim)概念**
内核将很少被使用到的内存换出到块设备(swap区)，以提供更多的主存，这种机制称为页交换(swapping)或者换页(paging)。如果一个很少使用的页的后备存储器是一个块设备(例如mmap文件映射)，则无需换出被修改的页，直接将页中数据与块设备同步，并腾出页帧。前面两种场景跟选择很少使用页的策略合在一起，统称为页面回收。页面回收主要是将页面交换到swap区或者同步到其他块设备。那么那种页可以被swap，那种页可以同步到块设备呢
