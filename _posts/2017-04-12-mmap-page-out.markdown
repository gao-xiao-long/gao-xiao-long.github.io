...
1. 内核检测到某个操作期间内存严重不足(主要是动态分配内存期间，如调用malloc()或new())，将调用trye_to_free_pages,该函数检查当前内存域中的所有页，并释放最不经常访问的那些（此操作可能会阻塞应用程序）
2. 一个名为kswapd后台守护进程，会定期检查内存的使用情况，当发现内存达到某一阈值时，会换出最不经常访问的那些(内核通过LRU算法管理页，会优先淘汰不活跃的页)，以防止内核在执行其他操作期间(如调用malloc()或new())发现内存不足。
