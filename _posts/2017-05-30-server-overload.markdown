...
常见的单机并发控制一般使用令牌桶或者漏桶方式，这两种方式只能够对单一维度进行控制，比如限制下发QPS。当下游系统由于某种原因导致处理时间变长后，可以处理的QPS也会随之降低，但是令牌桶或者漏斗的方式不能很好的反应这种变化。这时候可以考虑采用基于排队理论中的[Little's law](https://en.wikipedia.org/wiki/Little%27s_law) (利特儿法则)方式进行并发控制。 Little's law的公式为 L = λW。应用在计算机工程的队列中，可以认为：
* L：正在被处理的请求个数
* Lambda：平均的吞吐率
* W：平均响应时间
